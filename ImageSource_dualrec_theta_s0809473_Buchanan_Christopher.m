...
% each - imaginatively labelled with an L and R. Twiddling the r and theta knobs resizes and rotates
% head respectively in xy plane.
...
% radius (r) and orientation (theta) of head in xy plane.

r = 0.5;
theta = pi/2;

if abs(theta)>2*pi
    error('angle within 2*pi please')
end

%parametric solutions of a sphere in 2D Cartesian
...
for d=-N:N;
    for e=-N:N;
        for f=-N:N;
            ...
            ll = sqrt((Al^2)+(Bl^2)+(C^2));
            lr = sqrt((Ar^2)+(Br^2)+(C^2));
            tl = ti*ll;
            tr = ti*lr;
            w = abs(d)+abs(e)+abs(f);
...
xdata = [0,Lx,0,al,ar,Lx];
ydata = [0,0,Ly,bl,br,Ly];
