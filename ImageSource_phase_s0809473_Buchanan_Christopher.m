...
% this script is similar to ImageSource except it introduces random phase
% variation, s.t. it is not an 'energy IR' being generated, but a rather
% more conventional looking IR
...
% convert h from an energy IR to conventional IR (i.e. with phase
% characteristic)

l = length(h);
randsign = sign(rand(l,1)-0.5*(ones(l,1)));
h = h.*randsign;
...
Nstr = num2str(N);
params = strcat(Lxstr,'x',Lystr,'x',Lzstr,'_',alphastr,'_N',Nstr,'_phase');
