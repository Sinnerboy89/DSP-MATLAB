function [y] = allpassTV_quad_s0809473_Buchanan_Christopher(x,R,theta, delta,rate,Fs)
...
%the poles are at (R,theta) and (R,-theta). It is intended for use with the
%stereo phaser function, and is implemented on the right channel of stereo
%signal

N = length(x);
m = 1:N;
thetavec = theta+(delta*(1-sin(2*pi*m*rate/Fs)));
y = zeros(N,1);
